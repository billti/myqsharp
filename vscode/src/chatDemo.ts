...
import { getAuthSession, getRandomGuid, scopes } from './azure/workspaceActions';
...
async function chatRequest(token: string, question: string): Promise<QuantumChatResponse> {
    ...
    try {
        ...
        "https://api.quantum.microsoft.com/api/chat/completions",
        options
        );
...
const chatAgent: vscode.ChatAgent = async (
        ...
        token: vscode.CancellationToken): Promise<vscode.ChatAgentResult | void> => {
    ...
    if (useService) {
        const msaChatSession = await getAuthSession([scopes.chatApi,
        `VSCODE_TENANT:common`,
        `VSCODE_CLIENT_ID:${chatApp}`]);
        if (!msaChatSession) throw Error("Failed to get MSA chat token");
        const response = await chatRequest(msaChatSession.accessToken, prompt.content);
